...
export function calculateRanks(list) {
  const ranks = list.reduce((p, c, i) => {
    ...
    rank.stage1 =
      _.minBy(currentRuns.filter((x) => x.stage1 > 0).map((x) => x.stage1)) ||
      1000;
    rank.stage2 =
      _.minBy(currentRuns.filter((x) => x.stage2 > 0).map((x) => x.stage2)) ||
      1000;
    rank.stage3 =
      _.minBy(currentRuns.filter((x) => x.stage3 > 0).map((x) => x.stage3)) ||
      1000;
    rank.stage4 =
      _.maxBy(currentRuns.filter((x) => x.stage4 > 0).map((x) => x.stage4)) ||
      0;
    rank.stage5 =
      _.maxBy(currentRuns.filter((x) => x.stage5 > 0).map((x) => x.stage5)) ||
      0;
    rank.stage6 =
      _.maxBy(currentRuns.filter((x) => x.stage6 > 0).map((x) => x.stage6)) ||
      0;
  ...
  const topResultsPerCategory = categoryValues.map((category) => {
    ...
    const topResults = ranksPerCategory.reduce((p, c, i) => {
      ...
      p.stage6 = _.max([+c.stage6, +p.stage6]);
  ...
  const resultsPerCategory = categoryValues.map((category) => {
    ...
    const topResult = _.find(
      topResultsPerCategory,
    ...
    const results = ranksPerCategory.map((r) => {
      ...
      r.points6 = Math.floor((r.stage6 / topResult.stage6) * 1000);

      r.points =
        r.points1 + r.points2 + r.points3 + r.points4 + r.points5 + r.points6;
